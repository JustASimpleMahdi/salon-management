...
@section('main')
    <div class="w-[393px] min-h-[852px] bg-[#f8f4f1] mx-auto relative flex flex-col ">
        <!-- Header -->
        <div class="flx flex-col px-6">
            <h1 class="text-right text-[28px] font-bold text-[#2d211d] pt-[42px] leading-tight">نوبت‌ها</h1>

            <!-- Date label -->
            <p class="text-center text-sm text-[#6d5246] mt-[66px]">تاریخ:</p>

            <!-- Date picker row -->
            <div class="flex items-center justify-between gap-x-[24px] mt-[8px]">
                <a href="?date={{ $date->subDay()->format('Y/m/d') }}"
                ...
                <!-- Date input -->
                <form>
                    <input name="date" value="{{ $date->format('Y/m/d') }}"
                           class="w-30 text-center text-[19px] font-bold text-[#6d5246] bg-transparent border-none outline-none"/>
                </form>

                <a href="?date={{ $date->addDay()->format('Y/m/d') }}"
                   class="flex items-center justify-center w-[35px] h-[35px] rounded-full bg-white border border-[#707070]">
                    <svg width="13" height="24" viewBox="0 0 13 24" fill="none">
            ...
            <!-- Appointments list -->
            <div class="flex flex-col gap-4 mt-[63px] mb-[200px]">
                @foreach($appointments as $appointment)
                    <div class="relative w-[345px] bg-white rounded-3xl p-4">
                        <!-- Personnel names -->
                        <div class="text-right text-lg font-bold text-[#2d211d]">
                            {!! $appointment->personnels->pluck('fullname')->implode('<br>') !!}
                        </div>
                        <!-- Time range -->
                        <div class="text-right text-[13px] text-[#8d7366] mt-2">
                            {{ $appointment->start->format('H:i') }} - {{ $appointment->end->format('H:i') }}
                        </div>
                        <!-- Divider -->
                        <div class="my-2">
                            <svg width="297" height="1" viewBox="0 0 297 1" fill="none">
                                <path d="M0 0.5H297" stroke="#F0E8E3"/>
                            </svg>
                        </div>
                        <!-- Services -->
                        <div class="text-right text-xs text-[#b19a90] mt-2">
                        ...
                        @if($appointment->end->isFuture())
                            <div class="absolute left-4 bottom-4 cursor-pointer" onclick="openBottomDrawer({
            id: {{ $appointment->id }},
            ...
            deleteLink : '{{ route('manager.appointments.destroy',compact('appointment')) }}',
                                ...
                                <div class="relative w-[23px] h-[23px]">
                                    <div class="absolute inset-0 rounded-full bg-[#F4EEEA]"></div>
                                    <div class="absolute left-1/2 top-[5px] -translate-x-1/2 space-y-[2px]">
                                        <svg width="3" height="3" viewBox="0 0 3 3" fill="none">
                                            <circle cx="1.5" cy="1.5" r="1.5" fill="#6D5246"/>
                                        </svg>
                                        <svg width="3" height="3" viewBox="0 0 3 3" fill="none">
                                            <circle cx="1.5" cy="1.5" r="1.5" fill="#6D5246"/>
                                        </svg>
        ...
        <div class="fixed w-[393px] bottom-0 bg-white">
            <a href="{{ route('manager.appointments.create') }}"
               class="absolute left-1/2 -translate-x-1/2 bottom-[78px]">
                <div
                    class="w-[72px] h-[72px] rounded-full bg-gradient-to-br from-[#7c5a4d] via-[#5b4137] to-[#3a2923] shadow-[0_10px_25px_rgba(82,61,53,0.35)] flex items-center justify-center">
